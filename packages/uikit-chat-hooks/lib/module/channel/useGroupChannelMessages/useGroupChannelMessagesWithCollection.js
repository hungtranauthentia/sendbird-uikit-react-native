...
import { Logger, confirmAndMarkAsRead, isDifferentChannel, isMyMessage, isSendableMessage, useForceUpdate, useFreshCallback, useUniqHandlerId } from '@sendbird/uikit-utils';
...
export const useGroupChannelMessagesWithCollection = (sdk, channel, userId, options) => {
  ...
  const updateNewMessagesReceived = (source, messages) => {
    const incomingMessages = messages.filter(it => {
      ...
      return !isMyMessage(it, (_sdk$currentUser = sdk.currentUser) === null || _sdk$currentUser === void 0 ? void 0 : _sdk$currentUser.userId);
    });
  ...
  const init = useFreshCallback(async (startingPoint, limit, callback) => {
    ...
    collectionRef.current.initialize(MessageCollectionInitPolicy.CACHE_AND_REPLACE_BY_API).onCacheResult((err, messages) => {
      if (err) sdk.isCacheEnabled && Logger.error('[useGroupChannelMessagesWithCollection/onCacheResult]', err);else if (messages) {
      ...
      if (err) Logger.warn('[useGroupChannelMessagesWithCollection/onApiResult]', err);else if (messages) {
